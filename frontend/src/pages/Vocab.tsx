      ...
      <div className="max-w-4xl mx-auto px-6 py-10 w-full">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <input
            ...
            placeholder="搜索词、释义..."
            className="flex-1 min-w-[200px] bg-white border border-stone-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-stone-400"
          />
          <select
            ...
            className="bg-white border border-stone-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-stone-400"
          >
          ...
          <Link
            to="/review"
            className="px-3 py-2 rounded-md bg-stone-900 text-white text-sm hover:bg-stone-700"
          >
            开始复习
          </Link>
        </div>
        ...
        <ul className="space-y-3">
          {filtered.map((v) => {
            ...
            return (
              <li
                key={v.id}
                className="border border-stone-200 rounded-lg p-4 bg-white group"
              >
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    {mat && (
                      <Link
                        to={`/m/${mat.id}`}
                        className="text-xs text-stone-500 hover:text-stone-900 truncate max-w-[140px]"
                        title={mat.title}
                      >
                        {mat.title}
                      </Link>
                ...
                <p className="mt-1.5 text-sm text-stone-800 leading-relaxed">
                  {v.definition_zh}
                </p>
                {v.definition_en && (
  ...
  return (
    <span className="inline-flex gap-0.5 ml-1" title={`掌握度 ${level}/3`}>
      {[0, 1, 2].map((i) => (
        <span
          key={i}
          className={`w-1.5 h-1.5 rounded-full ${i < level ? 'bg-emerald-500' : 'bg-stone-200'}`}
        />
      ))}
    </span>
  );
